/**
 * Test Script for Claude Computer Use Integration
 * 
 * This script tests the direct API integration with Claude's computer use capabilities
 * following the example from Claude documentation.
 */
...
// Using Claude 3.7 Sonnet with mixed tool versions as per example
const model = 'claude-3-7-sonnet-20250219';
const betaFlag = 'computer-use-2025-01-24';

console.log(`Testing Claude Computer Use Integration with model: ${model}`);
console.log(`API Key (masked): ${apiKey ? apiKey.substring(0, 10) + '...' + apiKey.substring(apiKey.length - 5) : 'Not found'}`);

// Define the test prompt
const testPrompt = 'Create a text file named test.txt with the content "Hello from Claude Computer Use test!"';

// Define the computer use tools with consistent versions for Claude 3.7
...
// Run the test
testClaudeComputerUse().then(success => {
  if (success) {
